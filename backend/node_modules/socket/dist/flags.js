...
function getRawSpaceSizeFlags() {
  if (_rawSpaceSizeFlags === undefined) {
    const cli = vendor.meow({
      ...
      importMeta: {
        url: (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('flags.js', document.baseURI).href))
      }
    });
    _rawSpaceSizeFlags = {
...
function getMaxOldSpaceSizeFlag() {
  if (_maxOldSpaceSizeFlag === undefined) {
    ...
    if (!_maxOldSpaceSizeFlag) {
      const match = /(?<=--max-old-space-size=)\d+/.exec(constants.default.ENV.NODE_OPTIONS)?.[0];
...
function getMaxSemiSpaceSizeFlag() {
  if (_maxSemiSpaceSizeFlag === undefined) {
    ...
    if (!_maxSemiSpaceSizeFlag) {
      const match = /(?<=--max-semi-space-size=)\d+/.exec(constants.default.ENV.NODE_OPTIONS)?.[0];
